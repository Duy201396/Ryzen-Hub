...
if checkexpilot[exploit()] == false then
    game:GetService("Players").LocalPlayer:Kick("Not Support " .. exploit() .. " | Expilot mày đang dùng hỗ trợ đéo đâu " .. exploit())
else

end
--Thông báo
notis = require(game.ReplicatedStorage:WaitForChild("Notification"))
notis.new("<Color=Blue> Yt: Hdhackroblox<Color=/>"):Display()
notis.new("<Color=Green> Wellcome Bro To Ryzen Hub<Color=/>"):Display() 
notis.new("<Color=Red> Chúc Bạn Có Trải Nghiệm Tốt Nhất Khi Dùng Script<Color=/>"):Display() 
notis.new("<Color=White> Join Discord To Report Error<Color=/>"):Display()
-- chọn team 
...
local Window = Fluent:CreateWindow({
    Title = "Ryzen Hub",
    SubTitle = "Create Made By Yt:hdhackroblox",
    TabWidth = 160,
    Size = UDim2.fromOffset(480, 372),
...
local Tabs = {
    ...
    Stack = Window:AddTab({ Title = "Stack Farmming", Icon = "" }),
    Sub = Window:AddTab({ Title = "Sub Farmming", Icon = "" }),
}
Window:SelectTab(1)
...
--SHOP 
Tabs.Shop:AddSection("Shop Mua Võ")
Tabs.Shop:AddButton({
...
local ToggleFastAttackOld = Tabs.Shop:AddToggle("ToggleFastAttackOld", {Title = "Fast Attack", Default = true })
...
--stack farm
local DropdownSelectWeapon = Tabs.Stack:AddDropdown("DropdownSelectWeapon", {
    Title = "Select Weapon",
    Values = {'Melee','Sword'},
    Multi = false,
    Default = 1,
...
--level
local level = Tabs.Stack:AddSection("Level Farm")
...
--farm kata
local katakuri = Tabs.Stack:AddSection("Katakuri Farm")
...
local Mob_Kill_Cake_Prince = Tabs.Stack:AddParagraph({
...
-- Bone
local bonefarm = Tabs.Stack:AddSection("Bone Farmer")
...
local PosBone = CFrame.new(-9506.234375, 172.130615234375, 6117.0771484375)
spawn(function()
    while wait() do
        pcall(function()
...
local ToggleAutoRandumBone = Tabs.Stack:dToggle("ToggleAutoRandumBone", {Title = "Auto Random Bone", Default = _G.AutoRandumBone })
